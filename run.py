...
def visual(active_ids_, start, index, name, censoring_train):
    ...
    stds = 1 +torch.nn.functional.elu(means[:,:,-1])#1e-5 + softplus(means[:,:,-1])
    ...
    plt.plot(x_test, means.mean(1)[:,0]+2*stds_.mean(1),'o',label='Mean from ensemble', color='blue', zorder=2)
    plt.plot(x_test, means.mean(1)[:,0]-2*stds_.mean(1),'o',label='Mean from ensemble',  color='blue', zorder=2)
