...
def visual(active_ids_, start, index, name, censoring_train):
    ...
    if sum(cens_bald) != len(cens_bald):
        plt.plot(sorted, cens_bald[:,0][indices], color='#0F9D50', label = r'$I[y,\theta|x]$')
        plt.plot(sorted, unc_bald[:,0][indices], color='#4285F9', label = r'$I[l,\theta|x]$')
        if (sum(classes) != 0):
            print(classes.shape)
            plt.plot(sorted, classes[:,0][indices], color='#F4B400', label = 'Information from observed Values')
        plt.plot(sorted, cens_bald[:,0][indices]+unc_bald[:,0][indices], color='black', label = r'$I[y,l,\theta|x]$)')
        #plt.plot(sorted,cens_bald[indices])+torch.exp(unc_bald[indices]), color='black', label = 'Scoring function')
    else:
        plt.plot(sorted,unc_bald[indices], color='black',label = 'Scoring function')

    # add gray background for when observations are censored
    ax.grid(alpha=0.25)
    ...
    plt.ylabel("Acquisiton score", fontsize=font_size)
    #plt.ylim(None, 0.095)
    ...
    #plt.ylim(-3.5, 8.5)
    #plt.rcParams.update({'font.size': font_size})
    plt.plot(x_test, means.mean(1)[:,0],color='#DB4430', label = r'$\mu_i^*$', linewidth=3, markeredgewidth=1.5, markeredgecolor='black')
    ax.axvspan(-5, -3.04519382, facecolor='gray', alpha=0.15)
    ax.axvspan( -1.47879906, 0.10170736, facecolor='gray', alpha=0.15)
    ax.axvspan( 1.66810212, 3.2627202, facecolor='gray', alpha=0.15)
    #plt.fill_between(x_test.squeeze().numpy(), y1 = means.mean(1)[:,0]-2*stds_.mean(1),
    #                        y2 = means.mean(1)[:,0]+2*stds_.mean(1),alpha=0.15, color='#DB4430')#, label = r'$\mu_i(x_i)\pm 2 \sigma^{2*}_i(x_i)$')
                            
    if means.mean(1).shape[-1] > 2:
        plt.plot(x_test, means.mean(1)[:,2], label=r'$\mu_i$', linewidth=3, markeredgewidth=1.5, markeredgecolor='black')
        plt.plot(x_test, 2*torch.sigmoid(means.mean(1)[:,-1]), label=r'$\lambda_i$', linewidth=3, markeredgewidth=1.5, markeredgecolor='black')
    plt.ylim(-2.5, 9.0)
    ax.grid(alpha=0.25)
    ...
    #tau = 0.5*np.cos(2.0*x_test) + 2.0
    plt.plot(x_test, tau, color='gray', linestyle='dashed')
    plt.xlabel("x", fontsize=font_size)
...
dataset = "synth"
#x_train, y_train, censoring_train,x_val, y_val, x_test, y_test, censoring_test = get_dataset(dataset)
...
init_size = 10
query_size = 3
n_rounds = 150 # The first iteration is silent is silent.
trials = 2
...
c_cbald_mae = np.zeros([trials, n_rounds])
...
for k in range(0, trials):
    ...
    start.train()
    #cbald_[k,0], c_cbald_mae[k,0] = start.evaluate(x_test, y_test, censoring_test)
    ...
    for i in trange(1,n_rounds, desc='cbald'):
        ...
        start.train()
        #cbald_[k,i], c_cbald_mae[k,i] = start.evaluate(x_test, y_test, censoring_test)
