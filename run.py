...
def visual(active_ids_, start, index, name, censoring_train):
    ...
    scores, _, cens_bald, unc_bald = start.get_scores(5, plotting=True)
    ...
    if sum(cens_bald) != len(cens_bald):
        plt.plot(sorted,torch.exp(cens_bald[indices]), color='#0F9D50', label = 'Censored Part')
        plt.plot(sorted,torch.exp(unc_bald[indices]), color='#4285F9', label = 'Uncensored Part')
        plt.plot(sorted,torch.exp(cens_bald[indices])+torch.exp(unc_bald[indices]), color='black', label = 'Scoring function')
    else:
...
plt.plot(np.mean(duo_un),axis=0, label='duo_unc')
