...
def main(args):
    ...
    x_train, y_train, censoring_train, x_val, y_val, x_test, y_test, censoring_test, classes = get_dataset(args.dataset)
    ...
    for k in range(0, args.num_trials):
        ...
        for i in range(10):
            idx = np.random.choice(
                np.where(classes == i)[0], size=10, replace=False
            )
            initial_idx = np.concatenate((initial_idx, idx))

        active_ids[initial_idx] = True
        print(np.sum(active_ids))
        print(np.mean(active_ids))
