...
class ClassBaldSampling(Strategy):
    def __init__(self,X, Y, Cens,  ids, net_args, x_val, y_val, random_seed = 123):
        super(ClassBaldSampling, self).__init__(X, Y, Cens,  ids, net_args, x_val=x_val, y_val=y_val, random_seed=random_seed)
    ...
    def get_scores(self, n):
        ...
        t = (pt>0.1).float()
        return (t*torch.log(bald_unc)+(1-t)*torch.log(bald_cens)).detach().numpy(), idxs_unlabeled
