...
class GaussBaldSampling(Strategy):
    def __init__(self,X, Y, Cens,  ids, net_args, x_val, y_val, random_seed = 123):
        super(GaussBaldSampling, self).__init__(X, Y, Cens,  ids, net_args, x_val=x_val, y_val=y_val, random_seed=random_seed)
    ...
    def get_scores(self, n):
        ...
        return torch.log(bald_unc).detach().numpy(), idxs_unlabeled
